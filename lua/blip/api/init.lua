        ...
        sse_buffer, done, found_data_line = sse.process(sse_buffer, data, function(delta)
            ...
            if delta.tool_calls then
                for _, tc in ipairs(delta.tool_calls) do
                    ...
                    local fn = accumulated_tool_calls[idx]['function']
                    if tc['function'] then
                        if tc['function'].name then fn.name = fn.name .. tc['function'].name end
                        if tc['function'].arguments then fn.arguments = fn.arguments .. tc['function'].arguments end
                    end
                end
            end
        end)
